...
int main_file(const std::string& set_name, const std::vector<std::string>& files,
        Detector& detector, Tracker& tracker, WorldConfig& config)
{
    ...
    BusCounter counter(detector, tracker, config,
            [&files, &frameno]() -> nonstd::optional<std::tuple<cv::Mat, int>> {
...
int main() {
    ...
    //DetectorMot detector(0.1f);
    DetectionCache detection_cache(SOURCE_DIR "/data/detections_mot_train_mbnet.db", "uninitialised");
    CachedDetector cachedDetector(detection_cache, detector, 0.1f);

    std::ofstream results("results/uninitialised.txt");
    ...
    for (const auto& set: files) {
        ...
        //detector.set_file(trials_folder + set_name + "/det/det.txt");
        detection_cache.setTag(set_name);
        feature_cache.setTag(set_name);

        results.close();
        results.open(std::string("results/") + set_name + ".txt");

        main_file(set_name, set_files, cachedDetector, tracker, world_config);
    }
}